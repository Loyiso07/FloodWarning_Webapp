...
async function loadBridgeList() {
  ...
  bridges.forEach((bridge) => {
    ...
    row.innerHTML = `
      ...
      <button class="delete-btn" data-id="${bridge.id}">
        Delete
      </button>
    `;
...
document
  ...
  .addEventListener("submit", async (e) => {
    ...
    const body = {
      ...
      warning_threshold_cm: parseFloat(
        document.getElementById("warning_threshold_cm").value,
      ),

      danger_threshold_cm: parseFloat(
        document.getElementById("danger_threshold_cm").value,
      ),

      vibration_threshold_g: parseFloat(
        document.getElementById("vibration_threshold_g").value,
      ),
